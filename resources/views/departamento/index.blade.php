    ...
    <x-slot name="header">
        <div class="flex flex-nowarp justify-between">
            ...
            @can('departamento.create')
                <a href="{{ route('departamento.create') }}" class ="px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                    Nueva Regional
                </a>
            @endcan
        </div>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-xl sm:rounded-lg">
                @include('departamento._index-contenido', ['btnTexto' => ''])
            </div>
        </div>
    </div>
</x-app-layout>