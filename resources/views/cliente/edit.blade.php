<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-nowarp justify-between">
            ...
            <a href="{{ route('cliente.index') }}" class ="px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                Volver a Clientes
            </a>
        </div>
    </x-slot>
    ...
    <div class="py-12">
                ...
                <div class="md:col-span-1 flex justify-between">
                    <div class="px-4 sm:px-0">
                        ...
                        <p class="mt-1 text-sm text-gray-600">
                            Rellene y Modifique todos los campos para la información del cliente.
                        </p>
                    </div>
                <div class="px-4 sm:px-0">
            </div>
        ...
        <div class="mt-5 md:mt-0 md:col-span-2">
            <form action="{{ route('cliente.update', $cliente) }}" method="POST">
                @method('PUT')
                @include('cliente._formulario', ['btnTexto' => 'Actualizar'])
            </form>
        </div>
    </div>
